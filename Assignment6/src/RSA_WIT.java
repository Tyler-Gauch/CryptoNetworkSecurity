...
public class RSA_WIT {
	public static void main(String[] args) {
		...
		if (args.length != 3) {
			System.err
					.println("Provide 3 filenames: plain encrypted decrypted.");
			System.exit(-1);
		}

		RSA_WIT rsa = new RSA_WIT("977447", "649487", "6359"); // see the
																// constructor
																// for details.
		// p q publicKey
	...
	public void PrintParameters() {
		...
		System.out.println("|modulus: (" + modulus.bitLength() + "-bits) "
				+ modulus);
		System.out.println("|PU key:  (" + publicKey.bitLength() + "-bits) "
				+ publicKey);
		System.out.println("|PR key:  (" + privateKey.bitLength() + "-bits) "
				+ privateKey);
		System.out.println("+--------------------------------------");
	}
	 ...
	 * </pre>
	 * <P>
	 * To generate a (let's say) 20-bit prime number write an execute the
	 * following program:
	 * 
	 * <pre>
	 * import java.math.BigInteger;
	 ...
	 * 		BigInteger prime = BigInteger.probablePrime(20, random); // for a 20-bit
	 * 																	// random
	 * 																	// prime
	 * 																	// number
	...
	public RSA_WIT(String the_p, String the_q, String the_public_key) {
		...
		modulus = p.multiply(q); // 2N-bit number (common value in practice =
									// 2^16 + 1)
	 ...
	 * BigInteger into a byte[]. That means that a 40-bit BigInteger needs 6
	 * bytes to be stored. So, we are dumping into the file 6-byte BigIntegers
	 * (or 40/8 + 1 = 6).
	 */
	...
	/*
	 * Entry point of application that expect 3 file names. <OL> <LI> plaintext
	 * file name (could be a binary file as well) MUST EXIST! <LI> encrypted
	 * file name (encrypted content of the plaintext file) <LI> decrypted file
	 * name (decrypt the encrypted file yielding the plaintext's contents) </OL>
	 */
	/*
	 * public static void main(String[] args) {
	 * 
	...
	/**
	 * Encrypt/decrypt an input file on the fly. Encrypt the plaintext by a
	 * block and then decrypt that block and dump it in the output file. So, the
	 * input and the output file are identical.
	 * 
	 * @param inf
	 *            the name of the plaintext file.
	 * @param outf
	 *            the name of the file that will be produced and will have the
	 *            same content as the plaintext file.
	 */
	public void EncryptDecrypt(String inf, String outf) {
		...
		try {
			while ((w = in.read(buffer)) != 0) { // read a block of data at a
													// time
				if (w == NB) {
					BigInteger message = new BigInteger(1, buffer); // convert
																	// message
																	// into a
																	// number
																	// from
					...
					BigInteger decrypt = decrypt(encrypt); // decrypt right
															// after we encrypt.

					// System.out.println(message.toByteArray().length + " " +
					// encrypt.toByteArray().length + " " +
					// decrypt.toByteArray().length + " " + message + " " +
					// encrypt + " " + decrypt);

					byte[] dd = decrypt.toByteArray(); // get bytes of the
														// decrypted message

					if (dd.length > NB) { // message was originally NB number of
											// bytes
						byte[] gg = new byte[NB]; // if more than NB, get the
													// last NB bytes only.
						for (int r = 0; r < NB; r++)
							gg[r] = (byte) 0; // init array to 0s

						for (int t = NB - 1, q = dd.length - 1; t >= 0
								&& q >= 0; q--, t--) { // grab only the last NB
														// bytes
							gg[t] = dd[q];
						}
					...
					} else { // need to dump NB number of byte (pre-pend 0s
								// first)
						byte[] gg = new byte[NB];
						for (int r = 0; r < NB; r++)
							gg[r] = (byte) 0;

						for (int c = NB - 1, t = dd.length - 1; c >= 0
								&& t >= 0; c--, t--) {
							gg[c] = dd[t];
						}
						out.write(gg);
				...
				/*
				 * The loop checks for 0 (that we never get). We'll get -1 at
				 * the end of the file - and we break We'll get less than NB and
				 * the else part above will handle it.
				 * 
				 * else { // why in the world whould I ever get here? (input
				 * file is in blocks of NB...) BigInteger message = new
				 * BigInteger(1, buffer); BigInteger encrypt = encrypt(message);
	...
	public void EncryptFile(String inf, String outf) {
		...
		try {
			...
			BigInteger numberOfBytes = encrypt(new BigInteger(
					(fileIn.length() % NB) + ""));
			Write_BigInteger_to_file(out, numberOfBytes);
			while ((w = in.read(buffer)) != 0) {
				if (w == NB) {
					 ...
					 * " bits) message   = " + message);
					 * System.out.println("\t (" + encrypt.bitLength() +
					 * " bits) encrpyted = " + encrypt);
					 * System.out.println("\t (" + decrypt.bitLength() +
					 * " bits) decrypted = " + decrypt); System.out.println();
					 */
					...
					// In case we did not encrypt everything, because the last
					// block was not NB size.
					/*
					 * copy leftover from data array into t[] - (pad goes in
					 * front)
					 */
					int j = 0;
					byte[] t = new byte[NB];
					...
					for (int i = 0; i < w; i++, j++) { // copy data into t[]
						// System.out.println("To assign in t[" + j +
						// "] <- buffer["+i+"]");
						t[j] = buffer[i];
					}
	 ...
	 *            encrypted file name
	 * @param outfile
	 *            file containing the produced (decrypted) content.
	 */
	public void DecryptFile(String fname, String outfile) {
		...
		int ss = modulus.bitLength() / 8 + 1; // calculate how many BYTES the
												// module is (ss).
		byte[] tr = new byte[ss]; // modulus is 5 bytes (40 bits) - need one
									// more byte
		// tr will hold (ss) number of bytes from the encrypted file.
		try {
			int ret = -1;
			boolean firstTime = true; 
			BigInteger numberOfBytes = new BigInteger("0");
			while ((ret = in.read(tr)) == ss) { // go in a loop and read (ss)
												// number of bytes from the
												// encrypted file.
				BigInteger enc = new BigInteger(tr); // convert the encrypted
														// bytes into a
														// BigInteger
				...
				if(firstTime)
				{
					System.out.println("Number Of bytes: "+decrypt);
					numberOfBytes = decrypt;
					firstTime = false;
					continue;
				}

				byte[] data = decrypt.toByteArray(); // Convert the decrypted
														// BigInteger into an
														// array of bytes
				...
				if (data.length > NB) { // dump only the last NB bytes
					byte[] gg = new byte[NB]; // we encrypted the file using NB
												// number of bytes
												// (now we ended up with
												// data.length number of bytes)
												// We will use only the last NB
												// number of bytes.
					for (int r = 0; r < NB; r++)
						gg[r] = (byte) 0; // initialize array to all 0s

					for (int i = data.length - 1, j = NB - 1; j >= 0; --i, --j) {
						gg[j] = data[i];
					}

					out.write(gg); // dump gg[] into the output file (these are
									// the decrypted bytes.
					...
					if (in.available() > 0) { // if there is more data in the
												// encrypted file,
						//
						// ***** TODO *****
						//
						...
						// Copy the last NB number of bytes from data[] into the
						// newly created array.
						// Dump the newly created array into the file.
						byte[] gg = new byte[NB]; // we encrypted the file using
													// NB number of bytes
						...
						for (int i = data.length - 1, j = NB-1; j >= 0
								&& i >= 0; --i, --j) {
							gg[j] = data[i];
						}
					
						out.write(gg); // dump gg[] into the output file (these
										// are the decrypted bytes.
					} else {

						if(data.length < numberOfBytes.intValue())
						{
							for(int i = 0; i < numberOfBytes.intValue()-data.length; i++)
							{
								out.write(0);
							}
						}
